...
import type { ZBarSymbolType, ZBarConfigType } from './enum';

export class ImageScanner extends CppObject {
  static async create(wasmUrl:string= "/wasm/zbar.wasm"): Promise<ImageScanner> {
    const inst = await getInstance(wasmUrl);
