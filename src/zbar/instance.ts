...
let instPromise = async (wasmUrl: string) => {
  const res = await loadWasmInstance(wasmUrl, importObj);
  if (!res) {
...
export const getInstance = async (wasmUrl: string="/wasm/zbar.wasm" ): Promise<ZBar> => {
  return await instPromise(wasmUrl);
};
